...
app.post('/aggiungi', function(req,res){
    ...
    if (req.body.id_studente > 5 && req.body.id_studente < 100 && req.body.eta_studente > 18 && req.body.eta_studente < 32) {
        console.log(`OK: id ${req.body.id_studente} valido`);
        console.log(`OK: et ${req.body.eta_studente} valido`);
        res.send(`<h1>OK studente aggiunto con id [${req.body.id_studente}]</h1>`);
        res.send(`<h1>OK studente aggiunto con id [${req.body.eta_studente}]</h1>`);
    }  else {
        ...
        res.send(`<script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                  ...
                  res.send(`<script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                  <h1>ERROR: non posso aggiungere lo studente id [${req.body.eta_studente}]</h1>`);
    }
});
