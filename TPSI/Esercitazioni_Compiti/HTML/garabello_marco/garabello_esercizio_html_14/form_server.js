...
app.post('/aggiungi', function(req,res){
    ...
    if ((req.body.id_studente > 5 && req.body.id_studente < 100) && (req.body.eta_studente>=19&&req.body.eta_studente<=31)) {
        ...
        res.send(`<h1>OK studente aggiunto con id [${req.body.id_studente}] ed età [${req.body.eta_studente}]</h1>`);
    } else {
        console.log(`ERROR: id ${req.body.id_studente} o eta [${req.body.eta_studente}]non valido`);
        res.send(`<script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                  <h1>ERROR: non posso aggiungere lo studente id [${req.body.id_studente}] e eta [${req.body.eta_studente}]</h1>`);
    }
